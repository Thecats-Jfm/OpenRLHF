...
examples/train_ppo.py \
    --pretrain /paratera5-data/home/zhangdehao/models/Llama-2-7b-sft-model-ocra-500k \
...
if [[ ${1} != "slurm" ]]; then
    deepspeed  --master_port 29503 --hostfile examples/scripts/hostfile.txt $training_commands 2>&1 | tee $log_file
fi
