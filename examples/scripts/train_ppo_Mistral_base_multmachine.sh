...
examples/train_ppo.py \
    ...
    --reward_pretrain /paratera5-data/home/zhangdehao/models/MistralRM_mannual_initialization \
    --save_path $rundir \
    --save_steps -1 \
    --logging_steps 1 \
    --eval_steps -1 \
...
if [[ ${1} != "slurm" ]]; then
    deepspeed  --master_port 29513 --hostfile examples/scripts/hostfile.txt $training_commands 2>&1 | tee $log_file
fi
