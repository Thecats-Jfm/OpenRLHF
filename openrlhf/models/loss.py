...
class PolicyLoss(nn.Module):
    ...
    def __init__(self, clip_eps: float = 0.05) -> None:
        super().__init__()
    ...
    def forward(
        ...
        print(f"[CAT]Policy:shape of loss is {loss.shape}")
        # print(f"[CAT]surr1:{surr1}, surr2:{surr2}, loss:{loss}")
        loss = masked_mean(loss, action_mask, dim=-1).mean()
        # print(f"[CAT]Policy loss:{loss}")
        return loss
...
class ValueLoss(nn.Module):
    ...
    def __init__(self, clip_eps: float = 0.05) -> None:
        super().__init__()
        self.clip_eps = clip_eps
        print(f"[CAT]Value Loss clip_eps:{self.clip_eps}")
