...
class CriticPPOTrainer(PPOTrainer):
    def ppo_train(self):
        ...
        print("[CAT] CriticPPOTrainer: ppo_train")
        print(f"[CAT] len(replay_buffer): {len(self.replay_buffer)}, sample_batch_size: {self.replay_buffer.sample_batch_size}")
        ...
        print(f"[CAT] len(dataloader): {len(dataloader)}")
...
class CriticModelRayActor(BasePPORole):
    def init_model_from_pretrained(self, strategy: DeepspeedStrategy, pretrain, max_steps):
        print(f"[CAT]init critic model from {pretrain}")
        print(f"[CAT]max_steps: {max_steps}")
        print(f"[CAT]strategy: {strategy}")
