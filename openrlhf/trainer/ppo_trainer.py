...
class PPOTrainer(ABC):
    ...
    def __init__(
        ...
        **generate_kwargs,
        ...
        self.actor_loss_fn = PolicyLoss(0.05)
        self.critic_loss_fn = ValueLoss(0.05)
        ...
        if self.kl_target:
            print(f"[CAT]###ppo trainer kl_target###{kl_target}###{kl_horizon}###")
    ...
    def fit(
        ...
        print(f"[CAT]###ppo trainer fit###{prompts_dataloader.__len__()}###{pretrain_dataloader}###")

        update_timesteps = args.rollout_batch_size // (self.strategy.world_size * self.micro_rollout_batch_size)
        print(f"[CAT]###ppo trainer fit###{update_timesteps}###")
        print(f"[CAT]###ppo trainer fit###{args.rollout_batch_size}###{self.strategy.world_size}###{self.micro_rollout_batch_size}###")
        ...
        for episode in range(args.num_episodes):
            ...
            for rand_prompts in self.prompts_dataloader:
                ...
                if global_step % update_timesteps == 1:
                    print("[CAT] start log")
                    output = self.tokenizer.batch_decode(experience.sequences, skip_special_tokens=True)

                    self.strategy.print(f"###{output[0]}###")

                    # 打印 tensor 的尺寸和第一个位置的值
                    tensors_info = {
                    ...
                    for name, tensor in tensors_info.items():
                        if tensor is not None:
                            self.strategy.print(f"{name} shape: {tensor.shape}")
                            self.strategy.print(f"{name} first item: {tensor[0]}")
                    print('[CAT] end log')

                self.replay_buffer.append(experience)

                if global_step % update_timesteps == 0:
                    self.strategy.print(f"# Hello, I am replay buffer. and my length is {len(self.replay_buffer)}")
    ...
    def training_step(self, experience: Experience) -> Dict[str, float]:
        ...
        print('[CAT] no running training_step_reward')
